...
REPORT_STRINGS = {
    "en": {
        ...
        "sec_didnt_audit": "Scope of this edition",
        "didnt_intro": "The audit covers static rule matching, semantic-layer LLM analysis, and adversarial prompt fuzzing. Three classes of risk live beyond this edition's scope. We name them explicitly:",
        "didnt_runtime": "- **Runtime behavior.** Verifying what a skill actually does at runtime requires sandboxed execution. That layer ships in a future edition; today's report reflects what the skill states it will do, plus the LLM's read of how it would behave.",
        "didnt_chains": "- **Cross-skill composition.** When this skill is chained with others through a planner, the emergent state flow between skills is its own analysis surface. Out of scope for single-skill reports.",
        "didnt_external": "- **External payloads.** A skill that fetches and runs a remote script is flagged at the fetch step. The remote payload itself is audited as a follow-up once the sandbox layer is online.",
    ...
    "zh": {
        ...
        "sec_didnt_audit": "本期覆盖范围",
        "didnt_intro": "本审计覆盖三层：静态规则匹配、语义层 LLM 分析、对抗性 prompt fuzz。还有三类风险在本期范围之外，我们直接列清楚：",
        "didnt_runtime": "- **运行时行为。** 真实验证 skill 运行时行为需要沙盒执行能力，该层在后续版本上线。本期报告反映的是 skill 自述会做什么，加 LLM 对它行为的判读。",
        "didnt_chains": "- **跨 skill 组合。** Skill 通过 planner 串联时，skill 间的状态流转是独立的分析面。单 skill 报告范围之外。",
        "didnt_external": "- **外部 payload。** Skill 抓取并执行远程脚本的情况会在 fetch 步骤被标记。远程 payload 本身作为后续审计在沙盒层上线后单独发布。",
