...
Or configure it manually below. These track the **`master`** branch.
Swap `@master` for a release tag (e.g. `@v0.1.1`) once one is published
if you'd rather pin upgrades to an intentional version bump.

<details open>
<summary><b>Claude Code</b></summary>

```bash
claude mcp add tar-engine -- uvx --from "git+https://github.com/qingxuantang/tar-engine@master" tar-engine-mcp
```
